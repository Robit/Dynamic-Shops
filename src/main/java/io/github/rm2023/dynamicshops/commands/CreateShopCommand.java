...
public class CreateShopCommand implements CommandExecutor {
    ...
    public CommandResult execute(CommandSource src, CommandContext args) throws CommandException {
        ...
        if (min < 0) {
            Util.message(src, "The minimum price must be greater than 0!");
            return CommandResult.empty();
        }
        double max = args.<Double>getOne("maxPrice").get();
        if (max < min) {
            Util.message(src, "The maximum price must be greater than the minimum price!");
            return CommandResult.empty();
        }
        ...
        if(!(extraArgument.equals("") || extraArgument.equals("buyOnly") || extraArgument.equals("sellOnly")))
        {
            try {
                ...
                Util.message(src, "The optional argument must either be buyOnly, sellOnly, or a number specifying the initial price of the shop.");
                return CommandResult.empty();
            }
            if (initialPrice < min || initialPrice > max) {
                Util.message(src, "The initial price of the shop must be within the min and max price!");
                return CommandResult.empty();
            }
        }
        Player p = (Player) src;
        ...
        Shop shop = new ItemShop(name, null, min, max, k, !extraArgument.equals("canBuy"), !extraArgument.equals("canSell"), hand);
        if (!Double.isNaN(initialPrice)) {
            shop.setPrice(initialPrice);
        }
        initialPrice = shop.getPrice();
        Util.message(p, "You are creating a shop named " + name + " which will initially buy/sell the item stack (" + hand.getQuantity() + " " + hand.getType().getName() + ") in your hand for " + prefix + initialPrice + " with a minimum price of " + prefix + min + " and a maximum price of " + prefix + max + ".");
        if (!shop.getCanBuy()) {
